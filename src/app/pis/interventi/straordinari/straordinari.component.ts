...
import {
  DeleteInterventoStraordinarioGQL,
  InterventiStraordinariGQL,
  InterventiStraordinariSubscription,
...
export class InterventiStraordinariComponent implements OnInit {
  ...
  columns = [
    ...
    },
    {
      ...
      cell: (element: InterventoStraordinaioObj) =>
        `${
          element.data_inizio_lavori
            ? new Date(element.data_inizio_lavori).toLocaleDateString('it-IT', {
                timeZone: 'Europe/Rome',
              })
            : ''
        } - ${
          element.data_fine_lavori
            ? new Date(element.data_fine_lavori).toLocaleDateString('it-IT', {
                timeZone: 'Europe/Rome',
              })
            : ''
        }`,
    ...
    },
    {
      ...
      cell: (element: InterventoStraordinaioObj) =>
        `${
          element.data_inserimento
            ? new Date(element.data_inserimento).toLocaleDateString('it-IT', {
                timeZone: 'Europe/Rome',
              })
            : ''
        }`,
  ...
  applyFilter(event: any) {
    ...
    this.source = this._interventiStraordinariGQL
      .subscribe({
        where: {
          ...(this.model.data
            ? this.model.data.end == null
              ? { data_inserimento: { _eq: this.model.data.start } }
              : {
                  _and: [
                    ...(this.model.data.start
                      ? [{ data_inserimento: { _gte: this.model.data.start } }]
                      : []),
                    ...(this.model.data.end
                      ? [{ data_inserimento: { _lte: this.model.data.end } }]
                      : []),
                  ],
                }
            : {}),
          ...(this.model.data_inizio
            ? this.model.data_inizio.end == null
              ? { data_inizio_lavori: { _eq: this.model.data_inizio.start } }
              : {
                  _and: [
                    ...(this.model.data_inizio.start
                      ? [
                          {
                            data_inizio_lavori: {
                              _gte: this.model.data_inizio.start,
                            },
                          },
                        ]
                      : []),
                    ...(this.model.data_inizio.end
                      ? [
                          {
                            data_inizio_lavori: {
                              _lte: this.model.data_inizio.end,
                            },
                          },
                        ]
                      : []),
          ...
          ...(this.model.data_fine
            ? this.model.data_fine.end == null
              ? { data_fine_lavori: { _eq: this.model.data_fine.start } }
              : {
                  _and: [
                    ...(this.model.data_fine.start
                      ? [
                          {
                            data_fine_lavori: {
                              _gte: this.model.data_fine.start,
                            },
                          },
                        ]
                      : []),
                    ...(this.model.data_fine.end
                      ? [
                          {
                            data_fine_lavori: {
                              _lte: this.model.data_fine.end,
                            },
                          },
                        ]
                      : []),
      ...
      .subscribe(
        (response: SubscriptionResult<InterventiStraordinariSubscription>) => {
          this.dataSource.source!.next(
            response.data?.pis_intervento_straordinario
          );
  ...
  private _init() {
    return this._interventiStraordinariGQL
      .subscribe()
      .subscribe(
        (response: SubscriptionResult<InterventiStraordinariSubscription>) => {
  ...
  delete(row?: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this._deleteInterventoStraordinarioGQL
          .mutate({ id: row!.id })
          .subscribe({
            error: (e) => {
