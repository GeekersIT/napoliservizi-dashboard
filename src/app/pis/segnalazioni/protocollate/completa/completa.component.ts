...
import {
  ...
  Pis_Intervento_Constraint,
  Pis_Intervento_Update_Column,
  Pis_Attrezzature_Impiegate_Constraint,
  Pis_Attrezzature_Impiegate_Update_Column,
  Pis_Materiale_Dissesto_Constraint,
  Pis_Materiale_Dissesto_Update_Column,
  Pis_Segnaletica_Lasciata_Constraint,
  Pis_Segnaletica_Lasciata_Update_Column,
  Pis_Veicoli_Impiegati_Constraint,
  Pis_Veicoli_Impiegati_Update_Column,
...
export class SegnalazioniProtocollateCompletaComponent extends SegnalazioneEdit {
  ...
  async save(event: any) {
    ...
    let att_d = this.startData.intervento.attrezzature_impiegate.filter(
      (x: any) =>
        !this.model.intervento.attrezzature_impiegate
          .map((d: any) => d.id)
          .includes(x)
    );
    let mat_d = this.startData.intervento.materiali_dissesto.filter(
      (x: any) =>
        !this.model.intervento.materiali_dissesto
          .map((d: any) => d.id)
          .includes(x)
    );
    let seg_d = this.startData.intervento.segnaletica_lasciata.filter(
      (x: any) =>
        !this.model.intervento.segnaletica_lasciata
          .map((d: any) => d.id)
          .includes(x)
    );
    let vei_d = this.startData.intervento.veicoli.filter(
      (x: any) =>
        !this.model.intervento.veicoli.map((d: any) => d.id).includes(x)
    );

    const allegati = this.model.intervento.allegati
      ? await fileListToBase64(this.model.intervento.allegati)
      : [];
    let d = this.startData.intervento.allegati.filter(
    ...
    this._updateSegnalazioneGQL
      .mutate({
        ...
        segnalazione: {
          id: this.id,
          ...(event.type == 'def'
            ? { stato: Pis__Stato_Segnalazione_Enum.Completata }
            : { stato: this.model.stato }),
          intervento: {
            on_conflict: {
              constraint: Pis_Intervento_Constraint.InterventoPkey,
              update_columns: [
                Pis_Intervento_Update_Column.AssistenzaPm,
                Pis_Intervento_Update_Column.CondizioniTrafficoId,
                Pis_Intervento_Update_Column.DataFineLavori,
                Pis_Intervento_Update_Column.DataInizioLavori,
                Pis_Intervento_Update_Column.Difformita,
                Pis_Intervento_Update_Column.DissestoDifforme,
                Pis_Intervento_Update_Column.GiorniTrascorsiId,
                Pis_Intervento_Update_Column.Note,
              ],
            },
            data: {
              ...
              ...(n.length > 0 || d.length > 0
                ...
                : {}),
              ...(this.model.intervento.assistenza_pm
                ? { assistenza_pm: this.model.intervento.assistenza_pm }
                : {}),
              ...(this.model.intervento.condizioni_traffico
                ? {
                    condizioni_traffico_id:
                      this.model.intervento.condizioni_traffico.id,
                  }
                : {}),
              ...(this.model.intervento.data_fine_lavori
                ? { data_fine_lavori: this.model.intervento.data_fine_lavori }
                : {}),
              ...(this.model.intervento.data_inizio_lavori
                ...
                : {}),
              ...(this.model.intervento.difformita
                ? { difformita: this.model.intervento.difformita }
                : {}),
              ...(this.model.intervento.dissesto_difforme
                ? { dissesto_difforme: this.model.intervento.dissesto_difforme }
                : {}),
              ...(this.model.intervento.giorni_trascorsi
                ? {
                    giorni_trascorsi_id:
                      this.model.intervento.giorni_trascorsi.id,
                  }
                : {}),
              ...(this.model.intervento.note
                ? { note: this.model.intervento.note }
                : {}),
              attrezzature_impiegate: {
                on_conflict: {
                  constraint:
                    Pis_Attrezzature_Impiegate_Constraint.AttrezzatureImpiegatePkey,
                  update_columns: [
                    Pis_Attrezzature_Impiegate_Update_Column.Nome,
                    Pis_Attrezzature_Impiegate_Update_Column.Quantita,
                    Pis_Attrezzature_Impiegate_Update_Column.Delete,
                  ],
                },
                data: [
                  ...(this.model.intervento.attrezzature_impiegate
                    ? this.model.intervento.attrezzature_impiegate.map(
                        (attrezzatura: any) => {
                          return {
              ...
              materiali_dissesto: {
                on_conflict: {
                  constraint:
                    Pis_Materiale_Dissesto_Constraint.MaterialeDissestoPkey,
                  update_columns: [
                    Pis_Materiale_Dissesto_Update_Column.MaterialeId,
                    Pis_Materiale_Dissesto_Update_Column.Altro,
                    Pis_Materiale_Dissesto_Update_Column.Quantita,
                    Pis_Materiale_Dissesto_Update_Column.Delete,
                  ],
                },
                data: [
                  ...(this.model.intervento.materiali_dissesto
                    ? this.model.intervento.materiali_dissesto.map(
                        (materiale: any) => {
                          return {
                            ...(materiale.id ? { id: materiale.id } : {}),
                            ...(materiale.materiale
                              ? { materiale_id: materiale.materiale.id }
                              : {}),
                            ...(materiale.altro
                              ? { altro: materiale.altro }
                              : {}),
                            ...(materiale.quantita
              ...
              segnaletica_lasciata: {
                on_conflict: {
                  constraint:
                    Pis_Segnaletica_Lasciata_Constraint.SegnaleticaLasciataPkey,
                  update_columns: [
                    Pis_Segnaletica_Lasciata_Update_Column.Nome,
                    Pis_Segnaletica_Lasciata_Update_Column.Quantita,
                    Pis_Segnaletica_Lasciata_Update_Column.Delete,
                  ],
                },
                data: [
                  ...(this.model.intervento.segnaletica_lasciata
                    ? this.model.intervento.segnaletica_lasciata.map(
                        (segnaletica: any) => {
                          return {
                            ...(segnaletica.id ? { id: segnaletica.id } : {}),
                            ...(segnaletica.nome
                              ? { nome: segnaletica.nome }
                              : {}),
                            ...(segnaletica.quantita
                              ? { quantita: segnaletica.quantita }
                              : {}),
                          };
                        }
                      )
              ...
              veicoli_impiegati: {
                on_conflict: {
                  constraint:
                    Pis_Veicoli_Impiegati_Constraint.VeicoliImpiegatiPkey,
                  update_columns: [
                    Pis_Veicoli_Impiegati_Update_Column.Targa,
                    Pis_Veicoli_Impiegati_Update_Column.Delete,
                  ],
                },
                data: [
                  ...(this.model.intervento.veicoli
                    ? this.model.intervento.veicoli.map((veicolo: any) => {
                        return {
      ...
      .subscribe({
        next: async (result) => {
          ...
          const ret: any =
            result.data?.custom_insert_segnalazione?.segnalazione;
          ...
          this.startData.intervento.attrezzature_impiegate = [
            ...ret.intervento?.attrezzature_impiegate.map((e: any) => e.id)!,
          ];
          this.startData.intervento.materiali_dissesto = [
            ...ret.intervento?.materiali_dissesto.map((e: any) => e.id)!,
          ];
          this.startData.intervento.segnaletica_lasciata = [
            ...ret.intervento?.segnaletica_lasciata.map((e: any) => e.id)!,
          ];
          this.startData.intervento.veicoli = [
            ...ret.intervento?.veicoli_impiegati.map((e: any) => e.id)!,
          ];
        },
