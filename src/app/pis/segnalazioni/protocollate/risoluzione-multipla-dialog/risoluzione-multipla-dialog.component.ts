      ...
      templateOptions: {
        ...
        filter: (term: any, limit: number, offset: number, parent?: any) =>
          this._segnalazioneSelectGQL
            .watch({
              ...
              where: {
                _and: [
                  ...
                  { stato: { _eq: Pis__Stato_Segnalazione_Enum.Completata } },
                ],
            ...
            .valueChanges.pipe(map((result) => result.data?.pis_segnalazione)),
      },
    ...
    if (element.posizionamento_toponimo_punto_iniziale?.tipologia != null) {
      ...
      if (element.posizionamento_toponimo_punto_iniziale.specifica != null) {
        ...
        if (element.posizionamento_toponimo_punto_iniziale.civico != null)
          str =
            str + ' ' + element.posizionamento_toponimo_punto_iniziale.civico;
        if (element.posizionamento_toponimo_punto_iniziale.ipi != null)
          str = str + ' ' + element.posizionamento_toponimo_punto_iniziale.ipi;
        if (element.posizionamento_toponimo_punto_iniziale.km != null)
          str = str + ' ' + element.posizionamento_toponimo_punto_iniziale.km;
      }
      if (element.posizionamento_toponimo_punto_iniziale.connessione != null)
        str =
          str +
          ' ' +
    ...
    if (this.form.valid) {
      ...
      this._updateSegnalazioneGQL
        .mutate({
          ...
          segnalazione: this.data.segnalazioni.map(
            (segnalazione: SegnalazioneObj) => {
              return {
                id: segnalazione.id,
                stato: Pis__Stato_Segnalazione_Enum.Risolta,
                eventi: {
                  data: [
                    {
                      created_at: new Date(),
