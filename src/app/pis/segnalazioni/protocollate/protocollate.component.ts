...
export class SegnalazioniProtocollateComponent implements OnInit {
  ...
  columns = [{
      ...
      show: true,
      cell: (element: SegnalazioneObj) => `${element.protocollo?.numero ? element.protocollo.numero+" del "+new Date(element.protocollo.data).toLocaleString('it-IT', {timeZone: 'Europe/Rome'}): this._translateService.instant('In attesa')}`
    },{
      ...
      show: true,
      cell: (element: SegnalazioneObj) => `${element.stato ? element.stato : ''}`
    }];
  ...
  actionDisabled(row:any){
    return row.stato == _Stato_Segnalazione_Enum.Protocollazione || row.stato == _Stato_Segnalazione_Enum.Completata || row.stato == _Stato_Segnalazione_Enum.Risolta;
  }
