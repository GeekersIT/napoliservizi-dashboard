...
import {
  IndicatoriGQL,
  UpdateIndicatorePerformanceGQL,
...
export class EditComponent extends Dirty implements OnInit {
          ...
          asyncValidators: {
            formula: {
              expression: (c: any) => {
                ...
                formData.append('code', c.value);
                return this._http
                  .post('/editor/performance/', formData)
                  .pipe(map((e: any) => e.status));
              },
              message: (error: any, field: FormlyFieldConfig) => {
  ...
  constructor(
    ...
    private _updateIndicatorePerformanceGQL: UpdateIndicatorePerformanceGQL,
    private _indicatoriGQL: IndicatoriGQL,
    ...
    super();
    this.variabili = _indicatoriGQL.subscribe().pipe(
      map((e) =>
        e.data?.pms_formula.map((f) => {
  ...
  save(event: any) {
    this._updateIndicatorePerformanceGQL
      .mutate({
        performance: {
          id: this.model.id,
