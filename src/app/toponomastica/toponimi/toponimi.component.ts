...
export class ToponimiComponent implements OnInit {
  ...
  private _map(element: ToponimoObj) {
    const assegnazioni = element.assegnazioni.filter(
      (assegnazione) => assegnazione.fine_validita == null
    );
    const assegnazioni_vecchie = element.assegnazioni.filter(
      (assegnazione) => assegnazione.fine_validita != null
    );
    return {
      ...element,
            ...
            if (e.message.includes('Foreign key violation')) {
              this._snackBar.open(
                this._translateService.instant(
                  'Non è possibile eliminare il toponimo perchè è parte di un quartiere o lo è stato in passato.'
                ),
