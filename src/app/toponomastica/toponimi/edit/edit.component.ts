...
export class ToponimiEditComponent implements OnInit {
            ...
            let array = control.value
              .filter(
                (q: any) =>
                  q !== undefined &&
                  q !== null &&
                  q.municipalita !== undefined &&
                  q.quartiere !== undefined &&
                  q.municipalita !== null &&
                  q.quartiere !== null
              )
              .map((q: any) => {
                return q.municipalita.id + '-' + q.quartiere.id;
              });
  ...
  constructor(
    ...
    if (this.data) {
      ...
      this.model = this.data.dug
        ? { ...this.model, ...{ dug: data.dug } }
        : this.model;
      this.model = this.data.tipologia
        ? { ...this.model, ...{ tipologia: data.tipologia } }
        : this.model;
      this.model = {
        ...this.model,
  ...
  save() {
    ...
    const newAssegnazioni = this.model.assegnazioni
      ? this.model.assegnazioni.map((q: any) => {
          return q.municipalita.id + '-' + q.quartiere.id;
        })
      : [];
    let d = oldAssegnazioni.filter(
      (x) => !newAssegnazioni.includes(x.municipalita.id + '-' + x.quartiere.id)
    );
    let n = newAssegnazioni.filter(
      (x: any) =>
        !oldAssegnazioni
          .map((q) => q.municipalita.id + '-' + q.quartiere.id)
          .includes(x)
    );
    ...
    toponimo = this.model.dug
      ? { ...toponimo, ...{ dug_id: this.model.dug.id } }
      : toponimo;
    toponimo = this.model.tipologia
      ? { ...toponimo, ...{ tipologia_id: this.model.tipologia.id } }
      : toponimo;
              ...
              assegnazioni: {
                data: [
                  ...
                  ...n.map((q: any) => {
                    return {
                      inizio_validita: now,
                      municipalita_id: q.split('-')[0],
                      quartiere_id: q.split('-')[1],
                    };
                  }),
                ],
              },
