...
export class MunicipalitaComponent implements OnInit {
  ...
  fields: FormlyFieldConfig[] = [
    this._formFieldService.getQuartieri({
      key: 'quartiere',
      multiple: true,
      required: false,
    }),
  ...
  columns = [
    ...
    },
    {
      ...
      show: true,
      cell: (element: MunicipalitaObj) =>
        `${element.quartieri
          .map((quartiere) => quartiere.quartiere.nome)
          .join(', ')}`,
    },
  ];
  ...
  private _map(element: MunicipalitaObj) {
    const quartieri = element.quartieri.filter(
      (quartiere) => quartiere.fine_validita == null
    );
    const quartieri_vecchi = element.quartieri.filter(
      (quartiere) => quartiere.fine_validita != null
    );
  ...
  ngOnInit() {
    ...
    this.source = this._municipalitaGQL
      .subscribe()
            ...
            if (e.message.includes('Foreign key violation')) {
              this._snackBar.open(
                this._translateService.instant(
                  'Non è possibile eliminare la municipalità perché è associata ad altri elementi.'
                ),
