...
import { CiviciSelectGQL, ConnessioniGrafoSelectGQL, MunicipalitaSelectGQL, QuartiereSelectGQL, SostegniIpiSelectGQL, SpecificaPosizionamentoToponimoSelectGQL, TipologiaPosizionamentoToponimoSelectGQL, ToponimoNomeSelectGQL, ToponimoSelectGQL } from "src/app/_core/_services/generated/graphql";
    ...
    getQuartieri(params?: LocalizzazioneNamedParameters):FormlyFieldConfig{
      let field:FormlyFieldConfig = {
        ...
        ...(params?.root ? {
          hooks: {
            onInit: (field) => {
              field!.templateOptions!.parent = field?.parent?.fieldGroup?.find(f => f.key === params.root)?.formControl
            }
          },
        } :{}),
        expressionProperties: {
        ...
        fieldGroup: [
          ...
          fieldGroup: [{
            ...
            fieldGroup: [{
              ...
              templateOptions: {
                required:true,
                ...(params?.autosave ? {autosave: params.autosave[1]} : {}),
              ...
              templateOptions: {
                required:true,
                ...(params?.autosave ? {autosave: params.autosave[2]} : {}),
                displayWith: (element:any) => element!= null ? element.civico1 : '',
                filter: (term:any, limit:number, offset:number, parent?:any, ) => this._civiciSelectGQL.watch({
                  ...
                  ...(term && typeof term === 'string' ? { civico1: { _ilike: "%"+term+"%" } } : {} )
                }).valueChanges.pipe(
              ...
              ...(params?.root ? {
                hooks: {
                  onInit: (field) => {
                    field!.templateOptions!.parent = params.fields?.controls[params.root!];
                  }
                },
              } :{}),
              ...
              templateOptions: {
                required:true,
                ...(params?.autosave ? {autosave: params.autosave[3]} : {}),
                displayWith: (element:any) => element!= null ? element.matricola : '',
                filter: (term:any, limit:number, offset:number, parent?:any, ) => this._sostegniIpiSelectGQL.watch({
                  ...
                  ...(term && typeof term === 'string' ? { matricola: { _ilike: "%"+term+"%" } } : {} )
                }).valueChanges.pipe(
              ...
              templateOptions: {
                required:true,
                ...(params?.autosave ? {autosave: params.autosave[4]} : {}),
          ...
          templateOptions: {
            ...
            filter: (term:any, limit:number, offset:number, parent?:any, ) => this._connessioniGrafoSelectGQL.watch({
              ...
              map(data => term && typeof term === 'string' ? data.filter(t => t.nome.toLowerCase().indexOf(term.toLocaleLowerCase()) > -1) : data)
            ),
          },
          ...(params?.root ? {
            hooks: {
